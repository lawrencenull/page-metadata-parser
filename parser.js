...
const metadataRules = {
  ...
  icon_url: {
    ...
    processors: [
      (icon_url, context) => context.makeUrlAbsolute(context.url, icon_url)
    ]
  },
  ...
  image_url: {
    ...
    processors: [
      (image_url, context) => context.makeUrlAbsolute(context.url, image_url)
    ],
  },
  ...
  url: {
    ...
    processors: [
      (url, context) => context.makeUrlAbsolute(context.url, url)
    ]
  },
...
function getMetadata(doc, url, rules, makeUrlAbsolute, parseUrl) {
  const metadata = {};
  const context = {
    url,
    makeUrlAbsolute,
  ...
  Object.keys(ruleSet).map(metadataKey => {
    ...
    if(Array.isArray(metadataRule.rules)) {
      ...
      metadata[metadataKey] = getMetadata(doc, url, metadataRule, makeUrlAbsolute, parseUrl);
    }
  });
